SELECT * FROM TABLE(PKG_REPORT_VIEWS.GET_EMP_REPORT_DATA('2025-07', '20250729', 'HX_SELSPBM'));
...
create or replace PACKAGE PKG_REPORT_VIEWS AS
    ...
    FUNCTION GET_EMP_REPORT_DATA (
        ...
        P_BASE_DATE_YMD IN VARCHAR2, -- 예: '20250729'
...
create or replace PACKAGE BODY PKG_REPORT_VIEWS AS
    ...
    AS
        ...
        -- 변환된 날짜 문자열 미리 계산
        V_BASE_MONTH_TO_DATE_STR VARCHAR2(8) := TO_CHAR(TO_DATE(P_BASE_DATE_YMD, 'YYYYMMDD'), 'YYYYMMDD');
        V_BASE_MONTH_YM VARCHAR2(6) := SUBSTR(TRIM(REPLACE(P_BASE_DATE_YMD,'-','')),1,6);
    BEGIN
        FOR REC IN (
            WITH TMP AS (
                SELECT A.ENTER_CD, B.STATUS_CD
                 ...
                 WHERE A.ENTER_CD = P_ENTER_CD
                   ...
                   AND A.SABUN    = B.SABUN
                   AND A.SABUN IN (SELECT DISTINCT SABUN FROM THRM151_AUTH(P_ENTER_CD, 'A', P_SEARCH_SABUN, NVL(TRIM(NULL), TRIM('10'))))
                   AND P_BASE_DATE_YMD BETWEEN B.SDATE AND NVL(B.EDATE, '99991231')
                   AND A.ENTER_CD = C.ENTER_CD(+)
                   AND A.SABUN    = C.SABUN(+)
                   AND REPLACE(TRIM(V_BASE_MONTH_TO_DATE_STR), '-', '') BETWEEN C.SDATE(+) AND NVL(C.EDATE(+), '99991231')
                   AND B.STATUS_CD NOT LIKE 'RA%' -- 퇴직자 제외(RAA?)
                   AND (A.RET_YMD IS NULL OR A.RET_YMD = P_BASE_DATE_YMD) --당일 퇴직자만 표출
            ...
            SELECT 0 AS DETAIL
                 ...
                 , F_COM_GET_HOL_YN(A.ENTER_CD, P_BASE_DATE_YMD) HOL_YN
                 , F_TIM_GET_DAY_GNT_CD(A.ENTER_CD, A.SABUN, P_BASE_DATE_YMD) GNT_CD
                 , F_TIM_GET_DAY_GNT_NM(A.ENTER_CD, A.SABUN, P_BASE_DATE_YMD) GNT_NM
                 , F_TIM_GET_DAY_WORK_CD(A.ENTER_CD, A.SABUN, P_BASE_DATE_YMD) WORK_CD
                 , F_TIM_AGILE_SCHEDULE_YN(A.ENTER_CD, A.SABUN, P_BASE_DATE_YMD, P_BASE_DATE_YMD) FLEX_YN
                 , T9.SDATE
                 , T9.EDATE
                 , T9.YMD
                 , T9.SHM
                 , T9.EHM
                 , NVL(T9.SHM, CASE WHEN F_COM_GET_HOL_YN(A.ENTER_CD, P_BASE_DATE_YMD)='Y' THEN '' ELSE '0830' END) AS BASE_SHM
                 , NVL(T9.EHM, CASE WHEN F_COM_GET_HOL_YN(A.ENTER_CD, P_BASE_DATE_YMD)='Y' THEN '' ELSE '1730' END) AS BASE_EHM
              FROM (
                    SELECT Z.*
