        ...
        FROM THRM100 K
             ...
             LEFT OUTER JOIN (
                SELECT
                  ZZ.ENTER_CD,
                  ZZ.SABUN,
                  MAX(CASE WHEN ZZ.ORD_DETAIL_CD IN ('CJC_CHJ_06','CJC_CHJ_11')
                           THEN ZZ.ORD_YMD END) AS CHG_YMD,
                  MAX(CASE WHEN ZZ.ORD_DETAIL_CD = 'RFL_CD1'
                           THEN ZZ.ORD_YMD END) AS RFL_YMD
                FROM THRM191 ZZ
                WHERE ZZ.ENTER_CD = 'HX'
                GROUP BY ZZ.ENTER_CD, ZZ.SABUN
              ) ZZZ
                ON ZZZ.ENTER_CD = K.ENTER_CD
               AND ZZZ.SABUN    = K.SABUN
               AND K.ENTER_CD='HX'
