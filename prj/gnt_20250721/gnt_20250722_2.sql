...
LEFT OUTER JOIN (
    SELECT ENTER_CD, SABUN, APPL_SEQ, SDATE,EDATE, -- TTIM131의 모든 필요한 컬럼
           ROW_NUMBER() OVER (PARTITION BY ENTER_CD, SABUN ORDER BY APPL_SEQ DESC) as rn
    FROM TTIM131 
) T1
ON A.ENTER_CD = T1.ENTER_CD
AND A.SABUN = T1.SABUN
AND T1.rn = 1; -- 각 ENTER_CD, SABUN 조합에서 APPL_SEQ가 가장 높은 레코드만 선택
