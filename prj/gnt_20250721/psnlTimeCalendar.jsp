  ...
  function loadSheetData() {
   for (var i = sheet1.HeaderRows(); i < sheet1.HeaderRows() + sheet1.RowCount(); i++) {
    ...
    if (row.worktimeFrom !== '' && row.worktimeTo !== '') {
     ...
     var worktimeFromColSaveName = getTimeSaveName(parseFloat(row.worktimeFrom));
     var worktimeToColSaveName = getTimeSaveName(parseFloat(row.worktimeTo) - 0.5); // 종료 시간 30분 전까지 병합
     var leaveStartColIdx = sheet1.SaveNameCol(worktimeFromColSaveName);
     var leaveEndColIdx = sheet1.SaveNameCol(worktimeToColSaveName);
